...
import { PhoneIncoming, Radio, Send, Building2 } from "lucide-react";
import { CoverageGlobe } from "@/components/home/coverage-globe";
import { cn } from "@/lib/utils";

/* How PageMD works, as a bento.

   Adapted from `@efferd/features-6`. Three things changed and all three had to:

   1. Tokens. The block is written against shadcn's `primary` / `background` /
      `foreground` / `card` / `muted-foreground`. This project never ran
      `shadcn init` — it would rewrite globals.css and fight kajo — so none of
      those exist and every one of them rendered colourless. Remapped onto the
      usva roles: accent, bg/surface, ink, muted.
   2. `IconPlaceholder`. The registry ships it undefined, as a marker for you to
      substitute your icon set. Replaced with lucide, which the site already uses.
   3. The two hero visuals. The block's fourth card loads a dashboard screenshot
      off efferd's CDN and its fifth spins a WebGL globe (`cobe`). Neither says
      anything about medical paging, and an animated canvas on a scrolling page
      is a known cost here. Both are now drawn from the product: the structured
      page the demo actually emits, and the on-call routing that follows it.

   The two custom SVGs — stopwatch and lock — are the block's own, kept because
   they mean the right thing here. */

// Lifted verbatim from the transcript the demo plays, so the card and the demo
// below it never drift apart.
const PAGE_FIELDS = [
  ["Caller", "Maria Lopez · Nurse"],
  ["Patient", "James Carter"],
  ["For", "Dr. Patel"],
];

const ROUTING = [
  { name: "Dr. Patel", detail: "On call now", live: true },
  { name: "Dr. Nguyen", detail: "Off rotation", live: false },
  { name: "Dr. Ellis", detail: "Covered", live: false },
];

const features = [
  { id: "pickup", children: <PickupVisual />, className: "md:col-span-2" },
  { id: "phi", children: <PhiVisual />, className: "md:col-span-2" },
  {
    id: "parallel",
    children: <ParallelVisual />,
    className: "sm:col-span-2 md:col-span-2",
  },
  {
    id: "structured",
    children: <StructuredVisual />,
    className: "sm:col-span-2 md:col-span-3 p-0",
  },
  {
    id: "routing",
    children: <RoutingVisual />,
    className: "sm:col-span-2 md:col-span-3 p-0",
  },
  {
    id: "callers",
    children: <CallersVisual />,
...
function FeatureTitle({ className, ...props }: React.ComponentProps<"h3">) {
  return (
    <h3
      className={cn("font-medium text-ink text-lg", className)}
      {...props}
    />
  );
}
...
function PickupVisual() {
  return (
    <>
      <div className="relative mx-auto flex size-32 items-center justify-center rounded-full border-4 border-border border-dashed bg-bg shadow-xs outline outline-border outline-offset-4">
        <div className="absolute inset-0 z-10 scale-120 bg-radial from-ink/20 via-ink/5 to-transparent blur-xl" />
        <CustomTimerIcon className="size-14 fill-accent/90" />
      </div>

      <div className="relative mt-8 space-y-1.5 text-center">
        <FeatureTitle>Answered on the first ring</FeatureTitle>
        <FeatureDescription>
          No hold queue, no operator, no voicemail. Every inbound call is picked
...
function PhiVisual() {
  return (
    <>
      <div className="relative mx-auto flex size-32 items-center justify-center rounded-full border border-border bg-bg shadow-xs outline outline-border outline-offset-4">
        <CustomLockIcon className="size-24" />
        <div className="absolute inset-0 scale-120 bg-radial from-ink/15 via-ink/5 to-transparent blur-xl" />
      </div>

      <div className="relative mt-8 space-y-1.5 text-center">
        <FeatureTitle>Handled as PHI, start to finish</FeatureTitle>
        <FeatureDescription>
          PageMD signs a BAA as your Business Associate and encrypts every call
          in transit.
        </FeatureDescription>
      </div>
    </>
    ...
    <>
      <div className="min-h-32">
        <div className="absolute top-8 left-8 flex items-center gap-2">
          ...
          <div className="font-medium text-muted text-sm">4:55pm</div>
        </div>
        <ReportsChartsSvg className="-rotate-2 translate-x-[5%] scale-150" />
      </div>
      <div className="relative z-10 mt-8 space-y-1.5 text-center">
        <FeatureTitle>Calls in parallel, never in a queue</FeatureTitle>
        <FeatureDescription>
          Volume spikes at open, at lunch and at close. PageMD runs every
          conversation at once instead of stacking them.
        </FeatureDescription>
      </div>
    </>
  );
  ...
  return (
    <div className="grid h-full sm:grid-cols-2">
      <div className="relative z-10 space-y-6 py-8 ps-8 pe-2">
        <div className="flex size-12 items-center justify-center rounded-full border border-border bg-bg shadow-xs outline outline-border/80 outline-offset-2">
          <Send className="size-5 text-accent/80" />
        </div>
        <div className="space-y-2">
          <FeatureTitle className="text-base">
            Structured, not transcribed
          </FeatureTitle>
          <FeatureDescription>
            Caller, patient, reason and callback number come out as fields,
            built while the conversation is still happening.
          </FeatureDescription>
        </div>
      </div>
      {/* The page itself, in the shape the demo emits it. */}
      <div className="mask-b-from-90% mask-r-from-90% relative aspect-video sm:aspect-auto">
        <div className="absolute -right-1 -bottom-1 w-[19rem] max-w-full rounded-tl-md border border-border bg-bg p-1">
          <div className="space-y-3 rounded-tl-sm border border-border p-4">
            {PAGE_FIELDS.map(([label, value]) => (
              <div key={label} className="space-y-1">
                <div className="font-medium text-[0.68rem] text-muted uppercase tracking-wider">
                  {label}
                </div>
                <div className="truncate text-ink text-sm">{value}</div>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="grid sm:grid-cols-2">
      <div className="space-y-6 pt-8 pb-4 pl-8 sm:pb-8">
        <div className="flex size-12 items-center justify-center rounded-full border border-border bg-bg shadow-xs outline outline-border/80 outline-offset-2">
          <Radio className="size-5 text-accent/80" />
        </div>
        <div className="space-y-2">
          <FeatureTitle className="text-base">
          ...
          <FeatureDescription>
            The finished page goes to the covering provider, in a fixed format,
            without anyone paging the wrong person first.
          </FeatureDescription>
        </div>
      </div>
      <div className="mask-b-from-85% relative flex flex-col justify-center gap-2 px-8 pb-8 sm:py-8 sm:pl-2">
          ...
          >
            ...
            <div className="min-w-0">
              <div
                className={cn(
                  "truncate text-sm",
                  provider.live ? "text-ink" : "text-muted",
                )}
              >
                {provider.name}
              </div>
...
function CallersVisual() {
  return (
    <div className="relative grid items-center gap-8 sm:grid-cols-[minmax(0,1fr)_minmax(0,22rem)]">
      <div className="relative z-10 space-y-6 py-10 pl-8 sm:py-14">
        <div className="flex size-12 items-center justify-center rounded-full border border-border bg-bg shadow-xs outline outline-border/80 outline-offset-2">
          <Building2 className="size-5 text-accent/80" />
        </div>
        <div className="max-w-lg space-y-2">
          <FeatureTitle className="text-base">
            The calls come from outside your building
          </FeatureTitle>
          <FeatureDescription>
            Pharmacists verifying prescriptions, referring providers, hospitals,
            nurses returning calls. They all reach one line, and none of them
            wait on hold for someone to pick up.
          </FeatureDescription>
        </div>
      </div>
      {/* Owns its own height and crop geometry — those constants live in that
          component because they cannot cross the client boundary. */}
      <CoverageGlobe />
    </div>
  );
}
...
function CustomLockIcon(props: React.ComponentProps<"svg">) {
	return (
		...
		>
			<path
				className="fill-accent/50 stroke-accent/50"
		...
		>
			...
			<defs>
				<linearGradient
					...
					y2="123"
				>
					<stop className="text-accent/20" stopColor="currentColor" />
					<stop
						className="text-bg"
						offset="1"
